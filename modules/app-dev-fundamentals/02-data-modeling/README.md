...
> 已经能区分一对多和多对多关系？可以直接跳到 [03: 请求的生命周期](../03-request-lifecycle/README.md)。
...
就这样。数据建模的第一步不是画 ER 图（实体-关系图，一种数据结构的可视化图表）、不是写 SQL，而是用日常语言列出你的应用里有哪些"东西"。
