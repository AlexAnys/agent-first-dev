...
| | **延伸阅读** | |
| [Week 9](week9/README.md) | AI 系统监控（Agents Post-Deployment） | 生产环境监控与事件响应（阅读材料 + 自学实践） |
| [Week 10](week10/README.md) | AI 软件工程的未来（What's Next） | 行业展望与持续学习（阅读材料 + 自学实践） |

## 课程的逻辑线

课程按 **Understand → Build → Harden → Ship → Operate** 的顺序组织，覆盖完整的软件生命周期：
...
| **运维与未来** | Week 9-10 | 延伸阅读：生产环境监控、事件响应、行业趋势。 |

> **关于 Week 4-7 的 Starter Application**：你可能会注意到 Week 4-7 使用的是同一个 Notes/Action Items 应用。这是原课程的刻意设计——同一个"操练场"（playground），每周用不同的工具和方法论来操作它。重点不在应用本身的递进，而在于用同一个熟悉的代码库去体验 Claude Code（Week 4）、Warp 多代理（Week 5）、Semgrep 安全扫描（Week 6）和 Graphite 代码审查（Week 7）各自的视角和工作流。
...
按顺序完成 Week 1-8 的核心课程。每周大概需要 5-8 小时。Week 9-10 为延伸阅读，可选完成。这是最推荐的方式。
