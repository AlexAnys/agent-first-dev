> [← 模块首页](../README.md) · [← Week 3: MCP 服务器](../week3/README.md) · [Week 5: Warp Agentic 开发 →](../week5/README.md) · [术语表](../../../TERMINOLOGY.md)

# Week 4: 自主编码代理（Autonomous Coding Agent）

深入使用 Claude Code 的三大核心功能：CLAUDE.md、Slash Commands 和 SubAgents。

## 适合谁

- 想要系统化 AI 协作工作流的人
- 已经在使用 AI 编码工具但想提升效率的人

## 前置条件

- 完成 Week 1（提示技术基础）
- 了解基本的应用结构（Week 2）
- 安装 Claude Code CLI

## 预计时间

8-10 小时（深度实践），5-6 小时（快速路径）

## 学习顺序

按以下顺序学习本周内容：

1. **阅读中文导读** → [assignment-zh.md](assignment-zh.md) — 三大核心功能解释、实践建议
2. **完成课程练习** → [assignment.md](assignment.md) — 原课程作业（英文）
3. **记录学习笔记** → [writeup-zh.md](writeup-zh.md) — 结构化笔记模板
4. **进入下一周** → [Week 5: Warp Agentic 开发](../week5/README.md)

## 文件索引

| 文件 | 说明 |
|------|------|
| [assignment.md](assignment.md) | 原课程作业（英文） |
| [assignment-zh.md](assignment-zh.md) | 中文导读与概念解释 |
| [writeup.md](writeup.md) | 原课程提交模板（英文） |
| [writeup-zh.md](writeup-zh.md) | 学习笔记模板（中文） |

## 为什么这周特别重要

Week 4 是从"使用 AI 工具"到"设计 AI 工作流"的转折点。之前的练习是让 AI 帮你完成具体任务，这周开始学习如何搭建一套可复用的 AI 协作体系。

---

[← Week 3: MCP 服务器](../week3/README.md) · [Week 5: Warp Agentic 开发 →](../week5/README.md)
